...
@rate_limit(requests_per_minute=5)  # Limite rigoroso para registro
async def register(user_data: UserCreateSchema, db: Session = Depends(get_db_async)):
    """
    Registra novo usuário
    
    - **email**: Email válido (único)
    - **password**: Senha forte (8+ chars, maiúscula, minúscula, número, especial)
    - **name**: Nome completo
    """
    ...
    new_user = User(
        ...
        name=user_data.name,
        role=Role.USER.value,  # Padrão: usuário comum
        plan_tier="free",
        is_active=True
    )
...
async def login(credentials: UserLoginSchema, db: Session = Depends(get_db_async)):
    """
...
async def forgot_password(
    ...
    # Sempre retornar sucesso mesmo se email não existe (segurança)
    if not user:
    ...
    # Por enquanto, apenas logar o token (em desenvolvimento)
...
async def get_me(current_user = Depends(get_current_user), db: Session = Depends(get_db_async)):
    """
...
async def logout(
    ...
    if auth_header.startswith("Bearer "):
        token = auth_header[7:]
        # Adicionar à blacklist (expira em 1 hora)
...
async def create_admin_user(
    ...
    """
    Cria usuário administrador (apenas admin existente)
    """
    # Verificar se email já existe
    ...
    logger.warning(f"Novo admin criado por {admin_user.user_id}: {user_data.email}")
...
async def update_user_role(
    ...
    """
    Atualiza papel de usuário (apenas admin)
    """
    user = db.query(User).filter(User.id == user_id).first()
    if not user:
        raise HTTPException(status_code=404, detail="Usuário não encontrado")
    ...
    logger.warning(f"Role alterado por admin {admin_user.user_id}: User {user_id} {old_role} -> {new_role.value}")
