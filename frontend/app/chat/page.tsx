...
const WELCOME_MESSAGE: Message = {
  ...
  role: 'assistant',
  content:
    'Sou a **Lex**, assistente juridica do LexScan.\n\nPosso ajudar com prazos, fundamentacao normativa, estrategia processual, LGPD/cobranca etica e operacao do escritorio.\n\nRespondo com fontes oficiais quando disponiveis e sempre indico o que exige revisao do advogado responsavel. Escolha o modo (Rapida / Analise / Pesquisa) e envie sua pergunta.',
  timestamp: new Date(),
};

const SUGGESTED_PROMPTS = [
  'Quantos dias uteis para contestar no civel apos citacao (TJSP)? Fundamente com ressalvas.',
  'Checklist operacional para audiencia daqui a 3 dias.',
  'O que a LGPD exige antes de WhatsApp de cobranca de honorarios?',
  'Como montar aging de honorarios atrasados com tom etico OAB?',
];
    ...
    >
      ...
      >
        {isUser ? (
          ...
          <>
            {message.metadata?.contingency_mode || message.metadata?.ux_banner ? (
              <div className="mb-4 rounded-xl border border-sky-400/30 bg-sky-400/10 px-3 py-2 text-xs text-sky-100">
                {message.metadata?.ux_banner || 'Briefing fundamentado'}
                {' — '}
                resposta ancorada em fontes oficiais, sem conclusao de merito
                inventada. Revisao do advogado responsavel continua obrigatoria.
              </div>
            ) : null}
            {message.metadata?.model_degraded && !message.metadata?.contingency_mode ? (
